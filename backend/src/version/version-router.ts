 ...
 */
const LATEST_VERSION_CODE = Number(process.env['LATEST_VERSION_CODE'] ?? 101);
const LATEST_VERSION_NAME = process.env['LATEST_VERSION_NAME'] ?? '1.0';
/** En dessous de ce code → mise à jour forcée (réservé pour un usage futur). */
const MIN_VERSION_CODE = Number(process.env['MIN_VERSION_CODE'] ?? 1.1);
const STORE_URL =
